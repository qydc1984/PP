...
namespace ept
{
    ...
    void build_mtrr_map()
    {
        ...
        if (mtrr_cap.smrr_support == true)
        {
            ...
            if (current_phys_mask.valid && current_phys_base.type != mtrr_def_type.memory_type)
            {
                ...
                g_vmm_context->mtrr_info.enabled_memory_ranges++;
    ...
    bool setup_pml2_entry(__ept_state& ept_state, __ept_pde& entry, unsigned __int64 pfn)
    {
        ...
        {
            void* split_buffer = pool_manager::request_pool<void*>((VMM_CONTEXT*)g_vmm_context, pool_manager::INTENTION_SPLIT_PML2, true, sizeof(__ept_dynamic_split));
    ...
    void hook_write_relative_jump(unsigned __int8* target_buffer, unsigned __int64 destination_address, unsigned __int64 source_address)
    {
        ...
        __int32 jmp_value = (__int32)(destination_address - (source_address + 0x5));
    ...
    bool hook_instruction_memory(__ept_hooked_function_info* hooked_function_info, void* target_function, void** origin_function)
    {
        ...
        hooked_function_info->hook_size = (unsigned int)hooked_instructions_size;
    ...
    bool hook_function(__ept_state& ept_state, void* original_function, void* hooked_function, void** origin_function)
    {
        ...
        while (&ept_state.hooked_page_list != current->Flink)
        {
            ...
            if (hooked_page_info->pfn_of_hooked_page == GET_PFN(physical_address))
            {
                ...
                __ept_hooked_function_info* hooked_function_info = pool_manager::request_pool<__ept_hooked_function_info*>((VMM_CONTEXT*)g_vmm_context, pool_manager::INTENTION_TRACK_HOOKED_FUNCTIONS, TRUE, sizeof(__ept_hooked_function_info));
                ...
                hooked_function_info->trampoline_address = pool_manager::request_pool<unsigned __int8*>((VMM_CONTEXT*)g_vmm_context, pool_manager::INTENTION_EXEC_TRAMPOLINE, TRUE, 100);
        ...
        __ept_hooked_page_info* hooked_page_info = pool_manager::request_pool<__ept_hooked_page_info*>((VMM_CONTEXT*)g_vmm_context, pool_manager::INTENTION_TRACK_HOOKED_PAGES, true, sizeof(__ept_hooked_page_info));
        ...
        hooked_page_info->fake_page_contents = pool_manager::request_pool<unsigned __int8*>((VMM_CONTEXT*)g_vmm_context, pool_manager::INTENTION_FAKE_PAGE_CONTENTS, true, PAGE_SIZE);
        ...
        while (&ept_state.hooked_page_list != current_hooked_page->Flink)
        {
            ...
            if (hooked_page_info->pfn_of_hooked_page == GET_PFN(physical_address))
            {
                ...
                while (&hooked_page_info->hooked_functions_list != current_hooked_function->Flink)
                {
                    ...
                    //
                    if (function_page_offset == MASK_EPT_PML1_OFFSET((unsigned __int64)hooked_function_info->original_function_va))
                    {
